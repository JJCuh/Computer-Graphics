...
void init(GLFWwindow* window) {
	...
	brickTexture = Utils::loadTexture("brick1.jpg"); // for torus in the scene
	...
	N = normalize(glm::vec3(cameraX - pyrLocX, cameraY - pyrLocY, cameraZ - pyrLocZ));
	U = normalize(glm::vec3(-N.x, -N.y, N.z));
	V = normalize(glm::vec3(N.x, -N.y, -N.z));
}
...
void display(GLFWwindow* window, double currentTime) {
	...
	glm::mat4 r = glm::mat4(U.x, U.y, U.z, 0,
		V.x, V.y, V.z, 0,
		N.x, N.y, N.z, 0,
		0, 0, 0, 1);

	// builds and returns a matrix that performs both a rotation and a translation
	vMat = glm::translate(glm::mat4(1.0f), glm::vec3(-cameraX, -cameraY, -cameraZ)) * r;

	// vMat = glm::translate(glm::mat4(1.0f), glm::vec3(-cameraX, -cameraY, -cameraZ));

	// draw cube map
	// note that it requires a different rendering program
	...
	// buffer for texture coords not necessary
	glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);
...
int main(void) {
	...
	GLFWwindow* window = glfwCreateWindow(800, 800, "HW8", NULL, NULL);
