...
int shadowbufferwidth, shadowbufferheight;

float lookup(float x, float y, int width, int height)
{ // Q4
	shadowbufferwidth = width;
	shadowbufferheight = height;
	float t = textureProj(shadowTex, shadow_coord + vec4(x * 1.0/shadowbufferwidth * shadow_coord.w,
                                                         y * 1.0/shadowbufferheight * shadow_coord.w,
                                                         -0.01, 0.0));
	return t;
}
...
{	float shadowFactor=0.0;
	...
	shadowFactor += lookup(-1.5*swidth + o.x,  1.5*swidth - o.y, 1000, 1000);
	shadowFactor += lookup(-1.5*swidth + o.x, -0.5*swidth - o.y, 1000, 1000);
	shadowFactor += lookup( 0.5*swidth + o.x,  1.5*swidth - o.y, 1000, 1000);
	shadowFactor += lookup( 0.5*swidth + o.x, -0.5*swidth - o.y, 1000, 1000);
